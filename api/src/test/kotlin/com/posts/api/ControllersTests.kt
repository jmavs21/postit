...
class ControllersTests {
  ...
  inner class AuthControllerTests {
    ...
    fun `when api-auth POST with invalid email then 400`() {
      val response = testRestTemplate.postForEntity<String>(
        AUTH_API,
        AuthDtoReq("bob2@bob.com", "bob"),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.BAD_REQUEST)
      assertThat(
        JsonPath.parse(response.body).read<String>("$.email")
          .equals("the email doesn't exists", true)
      )
    }
    ...
    fun `when api-auth POST with invalid password then 400`() {
      val response = testRestTemplate.postForEntity<String>(
        AUTH_API,
        AuthDtoReq("bob@bob.com", "bob2"),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.BAD_REQUEST)
      assertThat(
        JsonPath.parse(response.body).read<String>("$.password")
          .equals("incorrect email or password", true)
      )
    }
  }
  ...
  inner class UsersControllerTests {
    ...
    fun `when api-users GET then users and 200`() {
      val response: ResponseEntity<Array<UserDto>> =
        testRestTemplate.getForEntity(USERS_API, Array<UserDto>::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
    ...
    fun `when api-users-{id} GET with valid id and headers then user and 200`() {
      val response: ResponseEntity<UserDto> = testRestTemplate.exchange(
        "$USERS_API/1",
        HttpMethod.GET,
    ...
    fun `when api-users-{id} GET with valid id but no header then 401`() {
      val response: ResponseEntity<String> =
        testRestTemplate.getForEntity("$USERS_API/1", String::class.java)
    ...
    fun `when api-users-me GET with no headers then 401`() {
      val response: ResponseEntity<String> =
        testRestTemplate.getForEntity("$USERS_API/me", String::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.UNAUTHORIZED)
    }
    ...
    fun `when api-users POST with valid new user then user token and 200`() {
      ...
      val response: ResponseEntity<UserDto> =
        testRestTemplate.postForEntity(USERS_API, userDtoReq, UserDto::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
    ...
    fun `when api-users POST with invalid name, email and password for new user then 400`() {
      val response: ResponseEntity<String> =
        testRestTemplate.postForEntity(USERS_API, UserCreateDtoReq("", "", ""), String::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.BAD_REQUEST)
      assertThat(
        JsonPath.parse(response.body).read<String>("$.name")
          .equals("size must be between 1 and 50", true)
      )
      assertThat(
        JsonPath.parse(response.body).read<String>("$.email")
          .equals("size must be between 1 and 50", true)
    ...
    fun `when api-users-{id} PUT with valid update user and headers then user token and 200`() {
      ...
      val response: ResponseEntity<UserDto> = testRestTemplate.exchange(
        "$USERS_API/2",
        HttpMethod.PUT,
        HttpEntity<Any>(userDtoReq, getAuthHeaders()),
        UserDto::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
      assertThat(response.body?.name).contains(userDtoReq.name)
    ...
    fun `when api-users-{id} PUT with invalid name but valid headers for update user then 400`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        "$USERS_API/2",
        HttpMethod.PUT,
        HttpEntity<Any>(UserUpdateDtoReq(""), getAuthHeaders()),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.BAD_REQUEST)
      assertThat(
    ...
    fun `when api-users-{id} PUT with valid name but invalid headers for update user then 401`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        "$USERS_API/2",
        HttpMethod.PUT,
        HttpEntity<Any>(UserUpdateDtoReq("izabel")),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.UNAUTHORIZED)
    }
  ...
  inner class PostsControllerTests {
    ...
    fun `when api-posts GET with defaults cursor and search and valid headers then posts and 200`() {
      val response: ResponseEntity<PostsDto> =
        testRestTemplate.getForEntity("$POSTS_API/?cursor=&search=", PostsDto::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
    ...
    fun `when api-posts GET with date cursor and default search and valid headers then posts and 200`() {
      val response: ResponseEntity<PostsDto> = testRestTemplate.getForEntity(
        "$POSTS_API/?cursor=2020-06-19T20:42:44&search=",
        PostsDto::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
      assertThat(response.body?.posts?.count()).isEqualTo(20)
    ...
    fun `when api-posts GET with default cursor and a search and valid headers then post and 200`() {
      val response: ResponseEntity<PostsDto> =
        testRestTemplate.getForEntity("$POSTS_API/?cursor=&search=down", PostsDto::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
    ...
    fun `when api-posts GET with cursor and search and valid headers then post and 200`() {
      val response: ResponseEntity<PostsDto> = testRestTemplate.getForEntity(
        "$POSTS_API/?cursor=2020-06-19T20:42:44&search=Distributed asymmetric structure",
        PostsDto::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
      assertThat(response.body?.posts?.count()).isEqualTo(1)
    ...
    fun `when api-posts-{id} GET with valid id then post and 200`() {
      val response: ResponseEntity<PostDto> =
        testRestTemplate.getForEntity("$POSTS_API/1", PostDto::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
    ...
    fun `when api-posts-{id} GET with invalid id then 400`() {
      val response: ResponseEntity<String> =
        testRestTemplate.getForEntity("$POSTS_API/0", String::class.java)
      assertThat(response.statusCode).isEqualTo(HttpStatus.NOT_FOUND)
    }
    ...
    fun `when api-posts POST with valid new post and headers then post and 201`() {
      ...
      val response: ResponseEntity<PostDto> = testRestTemplate.exchange(
        POSTS_API,
        HttpMethod.POST,
        HttpEntity<Any>(postDtoReq, getAuthHeaders()),
        PostDto::class.java
      )
    ...
    fun `when api-posts POST with invalid new post but valid headers then 400`() {
      ...
      assertThat(response.statusCode).isEqualTo(HttpStatus.BAD_REQUEST)
      assertThat(
        JsonPath.parse(response.body).read<String>("$.title")
          .equals("size must be between 1 and 50", true)
      )
      assertThat(
        JsonPath.parse(response.body).read<String>("$.text")
          .equals("size must be between 1 and 255", true)
      )
    }
    ...
    fun `when api-posts PUT with valid id, update post and headers then post and 200`() {
      ...
      val response: ResponseEntity<PostDto> = testRestTemplate.exchange(
        "$POSTS_API/8",
        HttpMethod.PUT,
        HttpEntity(postDtoReq, getAuthHeaders()),
        PostDto::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.OK)
      assertThat(response.body?.title).isEqualTo(postDtoReq.title)
    ...
    fun `when api-posts PUT with valid id, update post but invalid headers then 401`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        "$POSTS_API/8",
        HttpMethod.PUT,
        HttpEntity(PostDtoReq("Updated title", "Updated text")),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.UNAUTHORIZED)
    }
    ...
    fun `when api-posts PUT with invalid id but valid headers and update post then 404`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        "$POSTS_API/0",
        HttpMethod.PUT,
        HttpEntity(PostDtoReq("Updated title", "Updated text"), getAuthHeaders()),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.NOT_FOUND)
    }
  ...
  inner class VotesControllerTests {
    ...
    fun `when api-votes POST with up vote after a down vote and valid headers then vote and 201`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        VOTES_API,
        HttpMethod.POST,
        HttpEntity(VoteCreateDtoReq(4, false), getAuthHeaders()),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.CREATED)
      assertThat(response.body).isEqualTo("47")
      val response2: ResponseEntity<String> = testRestTemplate.exchange(
        VOTES_API,
        HttpMethod.POST,
        HttpEntity(VoteCreateDtoReq(4, true), getAuthHeaders()),
        String::class.java
      )
    ...
    fun `when api-votes POST with down vote after a up vote and valid headers then vote and 201`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        VOTES_API,
        HttpMethod.POST,
        HttpEntity(VoteCreateDtoReq(3, true), getAuthHeaders()),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.CREATED)
      ...
      val response2: ResponseEntity<String> = testRestTemplate.exchange(
        VOTES_API,
        HttpMethod.POST,
        HttpEntity(VoteCreateDtoReq(3, false), getAuthHeaders()),
        String::class.java
      )
      assertThat(response2.statusCode).isEqualTo(HttpStatus.CREATED)
    ...
    fun `when api-votes POST with valid vote but no headers then 401`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        VOTES_API,
        HttpMethod.POST,
        HttpEntity(VoteCreateDtoReq(4, true)),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.UNAUTHORIZED)
    }
    ...
    fun `when api-votes POST with invalid vote but valid headers then 0 and 201`() {
      val response: ResponseEntity<String> = testRestTemplate.exchange(
        VOTES_API,
        HttpMethod.POST,
        HttpEntity(VoteCreateDtoReq(0, true), getAuthHeaders()),
        String::class.java
      )
      assertThat(response.statusCode).isEqualTo(HttpStatus.CREATED)
